...
def _make_graphed_callables(
    ...
    *,
    ...
    def _run_warmup_forward(func_idx, func, callable_idx):
        ...
        # Call post-forward capture-time hooks
        hooks = capture_time_hooks[callable_idx]["forward_hooks"]
        hooks_with_kwargs = capture_time_hooks[callable_idx]["forward_hooks_with_kwargs"]
        for hook_id, hook in hooks.items():
            hook_args = (
                (func, args, kwargs, outputs) if hook_id in hooks_with_kwargs else (func, args, outputs)
            )
            if hook(*hook_args) is not None:
                raise RuntimeError(
    ...
    def _run_warmup_backward(func_idx, func, outputs, warmup_iter, callable_idx):
        ...
        # Call pre-backward capture-time hooks
        ...
        # Call post-backward capture-time hooks
    ...
    if _order is not None:  # pylint: disable=too-many-nested-blocks
            ...
            if c_id > 0:
                ...
                for l_no in list(reversed(range(_num_layers_per_chunk[m_chunk]))):
                    ...
                    if is_training:
                        # Call pre-backward capture-time hooks
                        for hook in capture_time_hooks[callable_idx]["backward_pre_hooks"].values():
                            grad_outputs = static_grad_outputs  ### TODO Check, this doesn't seem right
                            if hook(callables[callable_idx], grad_outputs) is not None:
                                raise RuntimeError(
                                    "capture_time_hooks backward_pre_hooks must not return a value "
                                    "(grad_output must not be modified via hook return)"
                                )
                        ...
                        # Call post-backward capture-time hooks
                        for hook in capture_time_hooks[callable_idx]["backward_hooks"].values():
                            grad_outputs = static_grad_outputs  ### TODO This doesn't seem right
                            if hook(callables[callable_idx], grad_inputs, grad_outputs) is not None:
                                raise RuntimeError(
                                    "capture_time_hooks backward_hooks must not return a value "
                                    "(grad_input must not be modified via hook return)"
                                )
        ...
        for func_idx, (func, args, kwargs, fwd_graph) in enumerate(
            ...
            # Call pre-forward capture-time hooks
            hooks = capture_time_hooks[func_idx]["forward_pre_hooks"]
            hooks_with_kwargs = capture_time_hooks[func_idx]["forward_pre_hooks_with_kwargs"]
            for hook_id, hook in hooks.items():
                hook_args = (
                    (func, args, kwargs) if hook_id in hooks_with_kwargs else (func, args)
                )
            ...
            # Call post-forward capture-time hooks
            hooks = capture_time_hooks[func_idx]["forward_hooks"]
            hooks_with_kwargs = capture_time_hooks[func_idx]["forward_hooks_with_kwargs"]
            for hook_id, hook in hooks.items():
                hook_args = (
                    (func, args, kwargs, outputs) if hook_id in hooks_with_kwargs else (func, args, outputs)
                )
        ...
        for static_input_surface, static_outputs, bwd_graph, bwd_dw_graph, bwd_idx in zip(
            ...
            if is_training:
                ...
                for hook in capture_time_hooks[bwd_idx]["backward_pre_hooks"].values():
                    if hook(callables[bwd_idx], static_grad_outputs) is not None:
                        raise RuntimeError(
                            "capture_time_hooks backward_pre_hooks must not return a value "
                            "(grad_output must not be modified via hook return)"
                        )
                ...
                for hook in capture_time_hooks[bwd_idx]["backward_hooks"].values():
                    grad_outputs = static_grad_outputs  ### TODO This doesn't seem right
                    if hook(callables[bwd_idx], grad_inputs, static_grad_outputs) is not None:
                        raise RuntimeError(
                            "capture_time_hooks backward_hooks must not return a value "
                            "(grad_input must not be modified via hook return)"
                        )
