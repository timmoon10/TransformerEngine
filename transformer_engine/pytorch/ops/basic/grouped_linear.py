...
class GroupedLinear(BasicOperation):
    ...
    def _functional_forward(
        ...
        weight: torch.Tensor,
        weight_splits: torch.Tensor,
        *,
        bias: Optional[torch.Tensor] = None,
        bias_splits: Optional[torch.Tensor] = None,
        device: Optional[torch.device] = None,
        dtype: Optional[torch.dtype] = None,
        with_quantized_compute: bool = False,
        input_quantizers: Optional[Iterable[Quantizer]] = None,
        weight_quantizers: Optional[Iterable[Quantizer]] = None,
        input_requires_grad: bool = True,
        weight_requires_grad: bool = True,
    ):

        # Check tensor splits
        if input_splits.dim() != 1:
            raise ValueError(
                "Expected input splits to be 1D tensor, "
                f"but got shape={tuple(input_splits.size())}"
            )
        num_splits = input_splits.size(0)
        if weight_splits.size() != (num_splits,):
            raise ValueError(
                f"Got input splits with shape={(num_splits,)} "
                f"and weight splits with shape={tuple(weight_splits.size())}"
            )
        if bias is not None:
            if bias_splits is None:
                raise ValueError("Got bias, but no bias splits")
            if bias_splits.size() != (num_splits,):
                raise ValueError(
                    f"Got input splits with shape={(num_splits,)} "
                    f"and bias splits with shape={tuple(bias_splits.size())}"
                )

        ### TODO Support quantization
        if with_quantized_compute:
            raise NotImplementedError("Quantized compute is not yet supported")

        # Check tensors
        x = maybe_dequantize(input, dtype)
        w = maybe_dequantize(weight, dtype)
        b = None if bias is None else maybe_dequantize(bias, dtype)

        # Split tensors
        xs = x.split(input_splits.tolist())
        ws = w.split(weight_splits.tolist())
        bs = [None] * num_splits if bias is None else b.split(bias_splits.to_list())

        # Perform GEMMs
        ys = []
        for i in range(num_splits):
            ys.append(torch.nn.functional.linear(xs[i], ws[i], bias=bs[i]))
        y = torch.cat(ys)

        # Return results and intermediate tensors
        extra_outputs = {"input": x, "weight": w}
        return y, extra_outputs
    ...
    def fuser_forward(
        ...
        *,
        ...
        # Unpack args
        ctx = basic_op_ctxs[0]
        input_splits = basic_op_extra_inputs[0][0]

        # Pack weight tensors
        weight: torch.Tensor
        if self.with_fused_weights:
            weight = self.weight
        else:
            weight = torch.cat(self._unfused_weights)
        weight_splits = torch.full(
            self.group_count,
            self.out_features,
            device="cpu",
            dtype=torch.int,
        ...
        weight_requires_grad = ctx.requires_grad and weight.requires_grad
        ...
        output, intermediate_tensors = GroupedLinear._functional_forward(
            input=input_,
